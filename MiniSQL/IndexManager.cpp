...
void BTree::InsertValue(void* pValue, int Block, int Offset)
{
	...
	if(m_sKeyType == "char") {                    //当key的类型为char时
		...
		tmpNode.InsertKey((int*)pValue, Block, Offset);	 //到达叶子节点时插入记录
	}
		...
		tmpNode.InsertKey((float*)pValue, Block, Offset);	 //到达叶子节点时插入记录
	}
...
void BTree::CreateBPlusTree()
{
	...
	((int*)p)[0] = m_iRoot;      //存入B树信息
	p += sizeof(int);
	((int*)p)[0] = m_iFirstLeaf;
	p += sizeof(int);
	((int*)p)[0] = MaxOffset;
	p += sizeof(int);
	((int*)p)[0] = EmptyOffset;
